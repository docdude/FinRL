...
class FinRLAdapter(BaseRLAdapter):
    ...
    def __init__(
        ...
        self.net_dims = net_dims or [64, 32]
    ...
    def predict(
        ...
        #   1. all sells first  (clamped to held shares)
        #   2. all buys passed through (trade_loop handles affordability)
        ...
        if turb_sell_all:
            ...
            # Pass through model's requested quantity; _execute_signals
            # in the trade loop handles cash affordability with live
            # quotes and refreshed cash after sells settle.
            ...
            for i in buy_indices:
                ...
                if price <= 0:
                    actions[i] = 0
                    continue
                actions[i] = abs(int(action_int[i]))
                self._stocks_cd[i] = 0
